  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'EMAIL_NOT_VERIFIED' || err.status === 403) {
        ...
        setError('Please verify your email before accessing ONCEClic.');
      } else {
            ...
            <div className="bg-rose-500/10 border border-rose-500/20 rounded-xl p-4 space-y-2 text-xs text-rose-300">
              ...
              {isUnverified && (
                <div className="pt-2 border-t border-rose-500/20 flex items-center justify-between">
                  ...
                  <Link to={`/verify-email?email=${encodeURIComponent(email)}`} className="text-slate-400 hover:text-slate-300">
                    Verify account &rarr;
                  </Link>
                </div>
              )}
